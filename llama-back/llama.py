...
async def create_site_code(site_id):
    ...
    code_prompt = f"""Write a complete code using HTML, CSS and Javascript to meet the following requirements: {site_data["requirements"]} and fill it with this content: {site_data["content"]}. Do not write additional commentaries. You're free to use libs as long as they can be used directly from a CDN. The content of the website should be written in Portuguese from Brazil. The website should be able to pass the Google Lighthouse benchmark.
    ...
    # content_prompt = f"""We generated a hmtl file based on the requirements for a website. Fill the following html: {formatted_html} """
