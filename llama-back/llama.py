...
from resources.sites.sites_service import find_site
...
async def extract_content(conversations):
    ...
    prompt_template = PromptTemplate.from_template(
        """[INST] Based on the following conversation between a person and a bot, extract only the content for the requested website. The response should be in portuguese: {conversation} [/INST]"""
    )
    prompt_formatted = prompt_template.format(
        conversation=prompt
    )
...
async def create_site_code(site_id):
    ...
    print('\formatted_software_requirements', formatted_software_requirements)
    print('\formatted_software_content', formatted_software_content)
    ...
    code_prompt = f"""Write a complete code using HTML, CSS and Javascript to meet the following requirements: {formatted_software_requirements}. Do not write anything besides the code. You're free to use libs as long as they can be used directly from a CDN. The content of the website should be written in Portuguese from Brazil.
    Use the following template: {arch_template}
    """
    ...
    generated_html_result = llm(code_prompt_formatted, max_tokens=10000)
    formatted_html = generated_html_result["choices"][0]["text"]

    print('\nformatted_html', formatted_html)

    content_prompt = f"""We generated a hmtl file based on the requirements for a website. Fill the following html: {formatted_html} with the content: {formatted_software_content}"""
